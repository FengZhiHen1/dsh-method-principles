...
| 是否写"破坏性操作"一关 | **写**，且写成自检问题而非动作禁令 | 由 2026-09 一次真实事故驱动（见下） |
| 唯一副本规则的例外 | 必须由**明确证据或用户**认定，模型自身信心不算 | 例外若无"认定方"，模型会自己给自己放行 |

事故依据：一次数据销毁事故中，无错误门的命令串把不可逆删除排在**静默失败**的步骤之后，且"完整性校验"写在删除之后（只能事后供出惨状）。因此该关的关键不是"更小心"，而是三件可检查的事：破坏性步骤独立成次调用、其依赖的前置结果必须已读过通过、校验必须在破坏之前。用户原提案中的"唯一副本禁止链式删除"经复核后**合并**进这一关——因为"链式"无统一机制（递归展开 / 链接穿透 / 级联删除 / 委托扩大是四件不同的事），而共同点是"真实受影响范围大于预期"，用自检问题即可覆盖。
